...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe Jev API key for AI-assisted organisation (Phase 2: auto-apply high-confidence tags, suggest mid-confidence chips). Empty disables all Jev calls. Also requires the per-user "AI-assisted organisation" setting (default off). When both are on, enrichment may send URL, title, site, and a ~1.5–2k character excerpt — never the full document, never other items. |
