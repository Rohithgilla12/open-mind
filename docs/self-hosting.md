...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe Jev API key for AI-assisted organisation (capture tags/chips, Lens re-rank, daily Drift scoring). Empty disables all Jev calls. Also requires the per-user "AI-assisted organisation" setting (default off). When both are on, Openmind may send URL, title, site, short excerpts/snippets, and a filtered 14-day activity summary — never the full document, never other items. See the AI section below for surfaces and blends. |
...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe [Jev](https://typesafe.ai) key. With the user setting **AI-assisted organisation** on: (1) post-save enrichment may call Jev with URL, title, site, and a short excerpt (~1.5–2k chars) for capture judgments — Phase 2 may auto-apply tags and show suggestion chips; (2) Lens/search may re-rank the top 30 hybrid hits when the client sends `rerank=true` (UI: **sort by relevance (beta)** on Lenses — not search-as-you-type). Re-rank sends the query plus ≤300-character snippets only; blend is `0.4·vector + 0.6·jev`. Ideal added latency ≤500 ms; Skip/timeout/API errors leave hybrid order unchanged and log `jev_decisions` with `surface=rerank`; (3) a daily River job scores Drift candidates with `worth_resurfacing` + `still_actionable` Nouls against a filtered summary of the last 14 days of saves (title/site/card type only — never full documents). Blend is `0.4·worth + 0.4·actionable + 0.2·2^(-ageDays/90)`; GET `/drift` reorders by stored score when fresh. Skip/API errors keep the recency heuristic and log `surface=drift`. Empty key → no call. |
