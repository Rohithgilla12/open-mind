...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe [Jev](https://typesafe.ai) key. With the user setting **AI-assisted organisation** on, post-save enrichment may call Jev with URL, title, site, and a short excerpt (~1.5–2k chars). Never the full document. Phase 2 may auto-apply high-confidence tags and surface mid-confidence suggestion chips (thresholds provisional); Skip/API errors leave the item unchanged. Empty key → no call. |
