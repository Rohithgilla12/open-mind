...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe Jev API key for AI-assisted organisation (Phase 1: shadow logging only). Empty disables all Jev calls. Also requires the per-user "AI-assisted organisation" setting (default off). When both are on, enrichment may send URL, title, site, and a ~1.5–2k character excerpt — never the full document, never other items. |
...
| `OPENMIND_TYPESAFE_API_KEY` | _(empty)_ | Optional TypeSafe [Jev](https://typesafe.ai) key. With the user setting **AI-assisted organisation** on, post-save enrichment may call Jev with URL, title, site, and a short excerpt (~1.5–2k chars). Never the full document. Phase 1 only writes `jev_decisions` (shadow); it does not change tags or card type. Empty key → no call. |
