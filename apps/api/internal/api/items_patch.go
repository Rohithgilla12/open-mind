...
// PatchItem edits an item owned by the caller. Only userTags is supported today:
// the supplied list fully replaces the item's user tags (an empty array clears
// them). Tags are canonicalised before storage. A body without userTags carries
// nothing to update and is rejected as a bad request. Unknown or cross-tenant
// ids resolve to 404 because the update is user-scoped. Returns the updated
